...
; want clauses that say: (not (c and c+1)) <=> ((not c) or (not c+1))
; ((-c v -(c+1)) ^ (-c v -(c+2)) ....
(defun at-most-one-color (n c k)
  (cond
    ;the last color is already paired from all other iterations 
    ((= c k) nil)
    (t (append (get-pairings c (+ c 1) n k) (at-most-one-color n (+ c 1) k)))
  )
)
...
;helper that makes a list of all pairings for one start-color
(defun get-pairings (curr-color pair-color n k)
  (cond
    ;if paired with the last color you're done 
    ((> pair-color k) nil)
    (t 
      (cons
        ;make one pair
        (list (neg-v (node2var n curr-color k)) (neg-v (node2var n pair-color k)))
        ;create the rest of the pairs
        (get-pairings curr-color (+ pair-color 1) n k)
      )
    )
  )
)
...
;This is just the union of clauses that have at least one color and at most one color
...
(defun generate-edge-clauses (e k)
  (edge-helper (car e) (car (cdr e)) 1 k)  
)

;(not (node1 curr-color) and (node2 curr-color)) <=> ((not node1 curr-color) or (not node2 curr-color))
(defun edge-helper (node1 node2 curr-color k)
  (cond
    ((> curr-color k) nil)
    (t 
      (cons
        (list (neg-v (node2var node1 curr-color k)) (neg-v (node2var node2 curr-color k)))
        (edge-helper node1 node2 (+ curr-color 1) k)
      )
    )
  )
)
